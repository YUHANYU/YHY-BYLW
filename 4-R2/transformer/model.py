...
class Sequence2Sequence():
    ...
    # 训练&验证
    def train_val(self, train_loader, val_loader, father_mark=None):
        """
        transformer模型训练-验证过程
        :param train_loader: 训练数据加载器
        :param val_loader: 验证数据加载器
        :param father_mark: 如果有编码父概念，则输入为父概念为none的词在的输入索引
        :return: 完成模型训练-验证过程
        """
        print('\n===开始训练&验证===')

        train_log = config.log
        val_log = config.log
        if config.log:  # 模型写入日志
            ...
            val_log = open(config.val_log, 'w', encoding='utf-8')
            val_log.write(
                '轮次：{epoch:3.0f}, '
                '批大小：{batch_size:3.0f}, '
                '损失：{loss:10.6f}\n\n, '
        ...
        for epoch in range(config.epochs):  # 按轮次训练数据
            ...
            total_word_correct = 0  # 本轮次所有批次数据中单词正确的个数
            epoch_start_time = datetime.datetime.now()  # 一个轮次模型的计算开始时间

            for batch_data in tqdm(train_loader, mininterval=1, ncols=1, desc='训练中...', leave=False):
                ...
                pre_tgt, _ = self.transformer(src_seq, tgt_seq)  # transformer模型预测结果

                real_tgt = tgt_seq[:, 1:].contiguous().view(-1)  # 构造实际的目标序列token
                loss, correct = self.criterion.cal_loss(real_tgt, pre_tgt)

                # with amp.scale_loss(loss, self.optimizer.optimizer) as scale_loss:
                #     scale_loss.backward()  # 尝试使用混度精度训练模型
            ...
            self.evaluate(val_loader, epoch, self.train_all_batch, val_log, father_mark)  # 每一个训练轮次结束，验证一次
    ...
    # 每个轮次训练结束，用验证数据验证模型
    def evaluate(self, data_loader, epoch, batch, val_log, father_mark):
        """
        每个轮次的数据训练完模型后，验证一次模型，根据指标确定要不要保留当模型保存点
        :param data_loader:
        :param epoch:
        :param batch:
        :param val_log:
        ...
        with torch.no_grad():  # 设置验证产生的损失不更新模型
            for batch_data in tqdm(data_loader, mininterval=1, ncols=1, desc='验证中...', leave=False):
                src_seq, tgt_seq = self.tool.batch_2_tensor(batch_data)  # 得到输入和目标序列 B*L B*L
                pre_tgt, _ = self.transformer(src_seq, tgt_seq)  # transformer模型预测结果
        ...
        if config.save_trained_model:
            ...
            if config.save_trained_model_type == 'all':
                ...
                model_name = config.save_model_checkpoint
    ...
    def infer(self, data_loader, source_lang, target_lang, tgt_max_len):
        ...
        if config.save_trained_model:  # 如果有预训练好的模型保存点
            words_data = torch.load(config.save_data)  # 加载训练&验证字符信息
        ...
        with open((os.path.abspath('.') + '/results/input_target_infer.txt'), 'w', encoding='utf-8') as f:
            for batch_dat in tqdm(data_loader, ncols=1, mininterval=1, disable=True, desc='Inferring...', leave=False):

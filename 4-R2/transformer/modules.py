...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, tgt_seq, father_seq=None, none_seq=None):
        """
        transformer模型前向计算流程
        :param src_seq: 输入批次序列 B*L
        :param tgt_seq: 目标批次序列 B*L
        :param father_seq: 父概念序列 B*L
        :param none_seq: 指示父概念序列是否为空的序列
        :return: 映射到目标序列单词表的输出 B*L*M
        """

        tgt_seq = tgt_seq[:, :-1]  # 删除目标序列的EOS符

        enc_src_out = self.encoder(src_seq)  # 编码器编码输入序列 B*L*(h*d=d_model)

        dec_src_tgt_out = self.decoder(tgt_seq, src_seq, enc_src_out)  # 解码器解码输入序列（训练时也计算目标序列）

        dec_out = dec_src_tgt_out

        word_pre = self.word_prob_map(dec_out)  # 映射到全部的单词 B*L*M

        output = word_pre  # 使用的是交叉熵，那就不需要做softmax了

        pre_word = output.view(-1, output.size(2))  # 维度变换，为了下一步的loss的计算 (B*L)*M

        return pre_word, output

    def _make_none_mask(self, none_seq, size):
        """
        制作父概念屏蔽位，为1则不屏蔽，为0则屏蔽
        :param none_seq: 指示每条父概念序列是否为none，为none则为0，反之为1
        :param size: 每个序列的最大长度
        :return:
        """
        none_mask = [[1 if i else 0 for _ in range(size)] for i in none_seq]
        none_mask = torch.FloatTensor(none_mask).to(config.device)

        return none_mask.view(len(none_seq), size, -1)  # TODO 这里还要仔细分析none_mask这样是否能有效屏蔽

...
class Tools():
    ...
    def batch_2_tensor(self, batch_data, none_mark=None):
        """
        把批次数据转化为tensor数据
        :param batch_data: B*L 该批次的数据
        :param none_mark: 父概念为none的标志
        :return:
        """
...
class Translator(object):
    ...
    def translate_batch(self, src_seq, src_pos, father_seq=None, father_pos=None, none_mask=None):
        ...
        def beam_decode_step(
                ...
                fa_seq=None, fa_enc=None, none_mask=None):
            """
            ...
            def make_none_mask(none_seq, size, map):
                """
                制作父概念屏蔽位，为1则不屏蔽，为0则屏蔽
                :param none_seq:
                :return:
                """
                none_seq = [none_seq[value] for key, value in map.items()]  # 去除不同屏蔽的位置
                none_seq = [i for i in none_seq for _ in range(config.beam_size)]
                none_mask = [[1 if i else 0 for _ in range(size)] for i in none_seq]
                none_mask = torch.FloatTensor(none_mask).to(config.device)

                return none_mask.view(len(none_seq), size, -1)
            ...
            def predict_word(
                ...
                if config.has_father:
                    # TODO 这里同时也要编码父概念，使其和decoder发生交互，然后再使用transformer.merge_father来做融合
                    dec_father_out = self.model.decoder(dec_seq, fa_seq, fa_enc)  # 得到对父概念的解码输出
                    none_mask = make_none_mask(none_mask, dec_father_out.size()[1], inst_idx_to_position_map)
                    dec_father_out *= none_mask  # 屏蔽掉为none的父概念
                    dec_src_father_tgt = torch.cat([dec_output, dec_father_out], dim=2)  # 级联输入序列和父概念
                    dec_output = torch.tanh(self.model.merger_father(dec_src_father_tgt))  # 融合二者，然后激活

                # 选择最后一步的解码序列, 本来也只有一步 [batch_size*beam_size, d]
                dec_output = dec_output[:, -1, :]  # Pick the last step: (bh * bm) * d_h
                # 所有词进行映射输出，得到每个词的概率 [batch_size*beam_size, M]
                word_prob = F.log_softmax(self.model.word_prob_map(dec_output), dim=1)  # 等于是当前每个词的生成概率
            ...
            def collect_active_inst_idx_list(inst_beams, word_prob, inst_idx_to_position_map):
                ...
                for inst_idx, inst_position in inst_idx_to_position_map.items():  # 循环索引和位置
                    is_inst_complete = inst_beams[inst_idx].advance(word_prob[inst_position])  # 当前序列是否预测到结束EOS
                    if not is_inst_complete:  # 如果没有完成
                        active_inst_idx_list += [inst_idx]

                return active_inst_idx_list

            n_active_inst = len(inst_idx_to_position_map)  # 当前要继续预测的批序列中的条数，大小在[0, batch_size]
            ...
            word_prob = predict_word(
                dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm,
                fa_seq=fa_seq, fa_enc=fa_enc, none_mask=none_mask)
        ...
        def collect_hypothesis_and_scores(inst_dec_beams, n_best):
            all_hyp, all_scores, all_scores_softmax, generate_p = [], [], [], []
            softmax = nn.Softmax(dim=0)
            for inst_idx in range(len(inst_dec_beams)):
                scores, tail_idxs = inst_dec_beams[inst_idx].sort_scores()
                all_scores += [scores[:n_best]]
                scores_softmax, tail_idxs_softmax = inst_dec_beams[inst_idx].sort_scores_softmax()  # 添加的
                all_scores_softmax += [scores_softmax[:n_best]]  # 添加的
                hyps = [inst_dec_beams[inst_idx].get_hypothesis(i) for i in tail_idxs[:n_best]]
                all_hyp += [hyps]
                scores_softmax = softmax(scores)  # 对scores做一维的softmax, 算出每一个的beam search概率
                generate_p += [round(scores_softmax[:n_best].cpu().item(), 3)]
            return all_hyp, all_scores, all_scores_softmax, generate_p  # hpy预测的序列，scores句子对应的得分，generate_p该句子的生成概率

        with torch.no_grad():
            #-- Encode  先编码
            src_seq, src_pos = src_seq.to(config.device), src_pos.to(config.device)  # 转移数据到指定设备上
            src_enc = self.model.encoder(src_seq)  # 编码输入序列

            if config.has_father:  # 编码父概念
                father_seq, father_pos = father_seq.to(config.device), father_pos.to(config.device)
                father_enc = self.model.encoder(father_seq)  # 编码父概念
                n_bm = config.beam_size  # beam search大小
                n_inst, len_s, d_h = father_enc.size()  # batch_size, max_len, d_model
                father_seq = father_seq.repeat(1, n_bm).view(n_inst * n_bm, len_s)  # [batch_size*beam_size, max_len]
                father_enc = father_enc.repeat(1, n_bm, 1).view(  # [batch_size*beam_size,max_len, d_model]
            ...
            #-- Bookkeeping for active or not  当前的序列是否预测结束
            active_inst_idx_list = list(range(n_inst))  # 批大小的list列表[0,2,...,batch_size]
            # 返回一个字典，{0:0,1:1,...,batch_size:batch_size} 用字典来记录哪个序列还没有预测结束，需要继续预测
            inst_idx_to_position_map = get_inst_idx_to_tensor_position_map(active_inst_idx_list)

            #-- Decode 解码阶段
            for len_dec_seq in range(1, self.tgt_max_len + 1):  # 在1~序列最大长度+1之内循环？为什么,因为是从1开始的，第一个为SOS

                active_inst_idx_list = beam_decode_step(
                    inst_dec_beams, len_dec_seq, src_seq, src_enc, inst_idx_to_position_map, n_bm,
                    fa_seq=father_seq if config.has_father else None,
                    fa_enc=father_enc if config.has_father else None,
                    none_mask=none_mask if config.has_father else None)

                if not active_inst_idx_list:  # 判断是否每个序列都解码到EOS结束符
                    break  # all instances have finished their path to <EOS>
                inst_idx_to_position_map_father = deepcopy(inst_idx_to_position_map)
                active_inst_idx_list_father = deepcopy(active_inst_idx_list)

                src_seq, src_enc, inst_idx_to_position_map = collate_active_info(
                    src_seq, src_enc, inst_idx_to_position_map, active_inst_idx_list)

                if config.has_father:  # 如果有父概念
                    father_seq, father_enc, _ = collate_active_info(
                        father_seq, father_enc, inst_idx_to_position_map_father, active_inst_idx_list_father)

        batch_hyp, batch_scores, batch_scores_softmax, batch_generate_p = collect_hypothesis_and_scores(
            inst_dec_beams, config.beam_search_left)

        return batch_hyp, batch_scores, batch_scores_softmax, batch_generate_p
...
class Beam():
    ...
    def __init__(self, size, device=False):
        ...
        self.scores_softmax = torch.zeros((size,), dtype=torch.float, device=device)  # 添加的，对预测的序列做一次softmax
        self.all_scores_softmax = []  # 添加的，记录最好得分softmax值

        # The backpointers at each time-step. 每个时间步后的指针
        self.prev_ks = []
    ...
    def advance(self, word_prob):
        ...
        softmax = nn.Softmax(dim=0)  # 添加的
        flat_beam_lk_softmax = softmax(flat_beam_lk)  # 添加的
        ...
        best_scores_softmax, best_scores_softmax_id = flat_beam_lk_softmax.topk(self.size, 0, True, True)  # 添加的
        self.all_scores_softmax.append(self.scores_softmax)  # 添加的
        self.scores_softmax = best_scores_softmax  # 添加的
    ...
    def sort_scores_softmax(self):
        return torch.sort(self.scores_softmax, 0, True)

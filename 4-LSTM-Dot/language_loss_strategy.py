...
def infer(config):
    ...
    write_info = []
    for model_epoch in range(config.epoch):
        start_time = time.time()

        enc_checkpoint = torch.load(  # 载入encoder保存点
            config.out_path + 'enc_' + str(model_epoch) + '.chkpt',
        ...
        dec_checkpoint = torch.load(  # 载入decoder保存点
            config.out_path + 'dec_' + str(model_epoch) + '.chkpt',
            map_location=lambda storage, loc:storage.cuda(0))
        ...
        *_, src_tgt_seq_infer = data_obj.word_2_index(
        ...
        infer_data_loader = DataLoader(
            src_tgt_seq_infer, 64, False, drop_last=False)
        tool = Tools(config)

        all_loss = []
        for batch_data in tqdm(infer_data_loader, desc='推理中...', leave=False):
        # for batch_data in infer_data_loader:
            ...
            # dec_hidden = enc_hidden[:config.enc_dec_layer]
            dec_hidden = (enc_hidden[0][:config.enc_dec_layer],
                          enc_hidden[1][:config.enc_dec_layer])
            max_tgt_len = max(tgt_len)
            all_dec_out = Variable(torch.zeros(max_tgt_len, this_batch_size, decoder.output_size))
...
def evaluate_model(all_loss, config, start_time):
    ...
    assert len(infer_con) * config.k == len(infer_con_k), '预测目标序列条数不是真实目标序列条数的{}倍！'.format(config.k)
    assert len(infer_con_k) == len(all_loss), '预测目标序列条数和预测目标序列损失值条数不一致！'

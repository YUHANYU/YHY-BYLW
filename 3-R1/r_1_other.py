...
class RetrievalModel():
    ...
    """
    def __init__(self, k, bio_nlp, use_edit, use_word_emb, use_info):
        """
        初始化函数
        :param k: 候选概念个数
        """
            ...
            for word in dia.split(' '):
                if word not in cons_word:  # 诊断文本的单词不在概念文本形成的单词字典中
                    ...
                    if max(every_distance) <= edit_distance \
                        ...
                        word_embedding = np.mat(self.bionlp.wv[word]).reshape(1, 200)
    ...
    def sif(self, train, val, infer, new_infer_dias, output_path, k_set):
        """
        ...
        for k in k_set:
            ...
            result_log.write('当k={}时，Acc={}，F-1={}, A@5={}, A@10={}，从头到尾的消耗时间为{}，每条耗时为{}'.
                             format(k, round(accuracy_score(y_true, y_pred), 3), round(f1, 3),
                                    round(acc_5, 3), round(acc_10, 3),
                                    take_time, each_items_time))
            result_log.write('\n')
            print('当k={}时，Acc={}，F-1={}, A@5={}, A@10={}，从头到尾的消耗时间为{}，每条耗时为{}'.
                             format(k, round(accuracy_score(y_true, y_pred), 3), round(f1, 3),
                                    round(acc_5, 3), round(acc_10, 3),
                                    take_time, each_items_time))
...
if __name__ == '__main__':
    ...
    for set_k in data_set:
        ...
        r1.sif(train, val, infer, new_infer_dia, output_path, k_set)
